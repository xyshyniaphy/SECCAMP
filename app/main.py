...
def setup_logging(config: Config) -> logging.Logger:
    ...
    log_file = config.log_dir / f"seccamp-{datetime.now().strftime('%Y-%m-%d')}.log"
...
def run_scrape(config: Config, logger: logging.Logger) -> int:
    ...
    logger.info("Scrape mode completed - placeholder")
    return 0


def run_full(config: Config, logger: logging.Logger) -> int:
    ...
    scrape_result = run_scrape(config, logger)
    if scrape_result != 0:
...
def main() -> int:
    ...
    # Run based on mode
    if args.mode == "scrape":
        return run_scrape(config, logger)
    else:
        return run_full(config, logger)
